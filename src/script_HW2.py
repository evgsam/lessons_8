...
while counter < max:
    # Отсылаем SYN-пакет
    syn = IP(dst=dest) / TCP(sport=random.randint(1025, 65500), dport=DPORT, flags='S')
    # Получаем SYN-ACK
    ...
    ack_packet = IP(dst=dest) / TCP(dport=DPORT, sport=syn_ack[TCP].sport, seq=syn_ack[TCP].ack, ack=(syn_ack[TCP].seq + 1), flags='A')
    send(ack_packet, verbose=False)

    # Отправляем HTTP GET-запрос
    http_request = IP(dst=dest) / TCP(dport=DPORT, sport=syn_ack[TCP].sport, seq=syn_ack[TCP].ack, ack=(syn_ack[TCP].seq + 1), flags='PA') / getStr
